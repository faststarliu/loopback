 ...
 *
 *  Author:
 *        J.L.Benjamin (Research Student, UCL)
 *
 *  Description:
...
int main(int argc, char* argv[]){
    ...
    while(1) {
        ...
        printf("[f] : PHY XS Loopback mode\n");
        printf("[g] : PCS Loopback mode\n");
        printf("[h] : PMA Loopback mode\n");
        printf("[i] : Disable current loopback\n");
        printf("[j] : Current Loopback Status\n");
        printf("[k] : Disable EDC\n");
        printf("[l] : Enable EDC\n");
...
//        printf("[q] : Interface 3 1G mode\n");
        ...
        if(option == 'a'){
            ...
            printf("**************\nRegister dump\n**************");fflush(stdout);
	    ael2005_write(f, dev, 1, 0XC01F, 0x4428);
	    ael2005_read(f, dev, 1, 0XC01F, &pma_status);
	    printf("\n\nPHY MODE: REG: 0x%04x\n", pma_status);
            printf("**************\nFinished\n**************\n");fflush(stdout);
        }
        else if(option == 'o'){
            printf("**************\nRegister reset\n**************");fflush(stdout);
	    ael2005_write(f, dev, 1, 0XC01F, 0x0428);
	    ael2005_read(f, dev, 1, 0XC01F, &pma_status);
	    printf("\n\nPHY MODE: REG: 0x%04x\n", pma_status);
            printf("**************\nFinished\n**************\n");fflush(stdout);
        }
/*        else if(option == 'n' || option == 'o' || option == 'p' || option == 'q'){
            ...
            for(port = 0; port < 4; port ++){
                ...
                printf("\nPort number: %d\n", port);fflush(stdout);
                ael2005_sys_loopback1(f, dev);
                ael2005_sleep(2);
                printf("Port: %d in loopback mode DONE\n", port);fflush(stdout);
            }
             printf("**************\nPCS Loopback Finished\n**************");fflush(stdout);
        }
        else if (option == 'h'){
            printf("**************\nPMA Loopback\n**************");fflush(stdout);
            for(port = 0; port < 4; port ++){
   	    ...
   	    printf("***********\nPMA Loopback Finished\n**************");fflush(stdout);
            ...
            for(port = 0; port < 4; port ++){
                ...
                ael2005_sys_loopback_dis(f, dev);
                ael2005_sleep(2);
            }
   	    printf("	LOOPBACK DISABLED SUCCESSFULLY\n**********************************************\n\n");fflush(stdout);
        }
        else if (option == 'j'){
            printf("**************Current Loopback Status**************");fflush(stdout);
 	    ael2005_read(f, dev, PMA_MDIO_DEVICE_ADDRESS, PMA_LOOPBACK_CTL, &pma_status);
            ael2005_read(f, dev, PCS_MDIO_DEVICE_ADDRESS, PCS_LOOPBACK_CTL, &pcs_status);
	    ael2005_read(f, dev, PHY_XS_MDIO_DEVICE_ADDRESS, PHY_XS_LOOPBACK_CTL, &phy_xs_status);
            if((pma_status) & 0x1){
            printf("\n\nTHE SYSTEM IS CURRENTLY IN PMA LOOPBACK MODE!\n\n");fflush(stdout);
       	    }
	    else if((pcs_status>>14) & 0x1){
            printf("\n\nTHE SYSTEM IS CURRENTLY IN PCS LOOPBACK MODE!\n\n");fflush(stdout);
       	    }
	    else if((phy_xs_status>>14) & 0x1){
            printf("\n\nTHE SYSTEM IS CURRENTLY IN PHY XS LOOPBACK MODE!\n\n");fflush(stdout);
       	    }
	    else{
            printf("\n\nNO LOOPBACKS IN SYSTEM!\n\n");fflush(stdout);
       	    }
	}
        else if (option == 'k'){
            printf("***************Disabling EDC...*************\n");fflush(stdout); 
 for(port = 0; port < 4; port ++){
                if(port == 0) dev = 2;
                if(port == 1) dev = 1;
                if(port == 2) dev = 0;
                if(port == 3) dev = 3;
                value = 0;
    	    ael2005_write(f, dev, 1, 0xC62B, 0x00f0);	   
    	    ael2005_write(f, dev, 1, 0xC62C, 0xffff);	
    	    ael2005_write(f, dev, 1, 0xC620, 0x007f);	
    	    ael2005_write(f, dev, 1, 0xC621, 0x0000);	
    	    ael2005_write(f, dev, 1, 0xC622, 0x0000);	
    	    ael2005_write(f, dev, 1, 0xC623, 0x0000);	
    	    ael2005_write(f, dev, 1, 0xC624, 0x0000);	
    	    ael2005_write(f, dev, 1, 0xC625, 0x0000);	
    	    ael2005_write(f, dev, 1, 0xC626, 0x0000);	
    	    ael2005_write(f, dev, 1, 0xC627, 0x0000);	
    	    ael2005_write(f, dev, 1, 0xC628, 0x0000);
	 ...
	 for(port = 0; port < 4; port ++){
                ...
                value = 0;
            printf("***************Enabling EDC...*************\n");fflush(stdout);
     	    ael2005_write(f, dev, 1, 0xC62B, 0x0000);
   	    ael2005_write(f, dev, 1, 0xC62C, 0x0000);	  
   	    printf("            EDC ENABLED!\n*******************************************\n\n");
	}
	}
        else if (option == 't'){
            printf("**************Dumping Feedforward Hffe registers**************\n");fflush(stdout);   
	 for(port = 0; port < 4; port ++){
                if(port == 0) dev = 2;
                if(port == 1) dev = 1;
                if(port == 2) dev = 0;
...
//DEBUG: WG HFFE  1,  0: 0xce14
//DEBUG: WG HFFE  3,  2: 0x1e19
//DEBUG: WG HFFE  5,  4: 0x00ba
//DEBUG: WG HFFE  7,  6: 0x007f
//DEBUG: WG HFFE  9,  8: 0xc4d8
//DEBUG: WG HFFE 11, 10: 0xe23c
//DEBUG: WG HFFE 11, 10: 0x0000
 
 printf("**************Write Group Feed forward Registers**************\n");
	    ael2005_read(f, dev, 1, 0XC620, &pma_status);
	    printf("\n\nDEBUG: WG HFFE  1,  0: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC621, &pma_status);
	    printf("\n\nDEBUG: WG HFFE  3,  2: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC622, &pma_status);
	    printf("\n\nDEBUG: WG HFFE  5,  4: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC623, &pma_status);
	    printf("\n\nDEBUG: WG HFFE  7,  6: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC624, &pma_status);
	    printf("\n\nDEBUG: WG HFFE  9,  8: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC625, &pma_status);
	    printf("\n\nDEBUG: WG HFFE 11, 10: 0x%04x\n\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC626, &pma_status);
	    printf("\n\nDEBUG: WG HFFE 11, 10: 0x%04x\n\n", pma_status);

 printf("**************Write Group Feed back Registers**************\n");
	    ael2005_read(f, dev, 1, 0XC627, &pma_status);
	    printf("\n\nDEBUG: WG HBFE  1,  0: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC628, &pma_status);
	    printf("\n\nDEBUG: WG HBFE  3,  2: 0x%04x\n\n", pma_status);

 printf("**************Read Group Feed Forward Registers**************\n");
	    ael2005_read(f, dev, 1, 0XC62C, &pma_status);
	    printf("\n\nDEBUG: HFFE OVERRIDE : 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC62D, &pma_status);
	    printf("\n\nDEBUG: RG HFFE  1,  0: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC62E, &pma_status);
	    printf("\n\nDEBUG: RG HFFE  3,  2: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC62F, &pma_status);
	    printf("\n\nDEBUG: RG HFFE  5,  4: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC630, &pma_status);
	    printf("\n\nDEBUG: RG HFFE  7,  6: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC631, &pma_status);
	    printf("\n\nDEBUG: RG HFFE  9,  8: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC632, &pma_status);
	    printf("\n\nDEBUG: RG HFFE 11, 10: 0x%04x\n\n", pma_status);
	    
 printf("**************Read Group Feed Backward Registers**************\n");
	    ael2005_read(f, dev, 1, 0XC633, &pma_status);
	    printf("\n\nDEBUG: RG HFBE 02, 01: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC634, &pma_status);
	    printf("\n\nDEBUG: RG HFBE 04, 03: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC637, &pma_status);
	    printf("\n\nDEBUG: RG HFFE 11, 10: 0x%04x\n", pma_status);
	    ael2005_read(f, dev, 1, 0XC638, &pma_status);
	    printf("\n\nDEBUG: RG HFFE 11, 10: 0x%04x\n", pma_status);

	       	    printf("	REGISTER VALUES DUMPED SUCCESSFULLY\n**********************************************\n\n");fflush(stdout);
        }
}
        else if (option == 'x'){
            break;
...
// PHY XS Loopback Registers
int ael2005_sys_loopback(int file_ptr, uint32_t dev){
    ael2005_write(file_ptr, dev, PHY_XS_MDIO_DEVICE_ADDRESS, PHY_XS_LOOPBACK_CTL, PHY_XS_LOOPBACK_VALUE);
    return 0;
}

// PCS Loopback Registers
int ael2005_sys_loopback1(int file_ptr, uint32_t dev){
    ael2005_write(file_ptr, dev, PCS_MDIO_DEVICE_ADDRESS, PCS_LOOPBACK_CTL, PCS_CTL_RESET);
    ael2005_write(file_ptr, dev, PCS_MDIO_DEVICE_ADDRESS, PCS_LOOPBACK_CTL, PCS_LOOPBACK_VALUE);
    return 0;
}

// PMA Loopback Registers
int ael2005_sys_loopback2(int file_ptr, uint32_t dev){
    uint16_t pma_status;
    ael2005_write(file_ptr, dev, PMA_MDIO_DEVICE_ADDRESS, PMA_LOOPBACK_CTL, PMA_CTL_RESET);
    ael2005_write(file_ptr, dev, PMA_MDIO_DEVICE_ADDRESS, PMA_LOOPBACK_CTL, PMA_LOOPBACK_VALUE);
//    ael2005_read(file_ptr, dev, PMA_MDIO_DEVICE_ADDRESS, PMA_LOOPBACK_CTL, &pma_status);
//    printf("\n\nDEBUG: stt pma: 0x%04x\n", pma_status);
    return 0;
}

int ael2005_sys_loopback_dis(int file_ptr, uint32_t dev){
    ael2005_write(file_ptr, dev, PMA_MDIO_DEVICE_ADDRESS, PMA_LOOPBACK_CTL, 0x8000);
    ael2005_write(file_ptr, dev, PCS_MDIO_DEVICE_ADDRESS, PCS_LOOPBACK_CTL, 0x8000);
    ael2005_write(file_ptr, dev, PHY_XS_MDIO_DEVICE_ADDRESS, PHY_XS_LOOPBACK_CTL, 0x0000);
    return 0;
}

int ael2005_reset(int file_ptr, uint32_t dev){
    ael2005_write(file_ptr, dev, PMA_MDIO_DEVICE_ADDRESS, PMA_CTL_REGISTER_ADDRESS, PMA_CTL_RESET);
    return 0;
}
